...
public class UserActionServiceImpl implements UserActionService {
    ...
    public void save(UserAction userAction) {
        ...
        if (maybeUserAction.isPresent()) {
            UserAction oldUser = maybeUserAction.get();
            if (userAction.getUserScore() > oldUser.getUserScore()) {
                oldUser.setUserScore(userAction.getUserScore());
                userActionRepository.save(oldUser);

                log.debug("Обновлен вес пользователя: userId={}, eventId={}, score={}",
                        oldUser.getUserId(), oldUser.getEventId(), oldUser.getUserScore());
            }
        } else {
