...
public class RecommendationServiceImpl implements RecommendationService {
    ...
    public Stream<RecommendedEventProto> getRecommendationsForUser(UserPredictionsRequestProto request) {
        ...
        if (recentUserEvents.isEmpty()) {
            log.info("У пользователя с ID={} нет взаимодействий, рекомендации не найдены", userId);
            return Stream.empty();
        }
        ...
        if (candidates.isEmpty()) {
            log.info("Для пользователя с ID={} нет похожих мероприятий", userId);
            return Stream.empty();
        }
        ...
        log.info("Сформировано {} рекомендаций для пользователя ID={}", result.size(), userId);
    ...
    public Stream<RecommendedEventProto> getInteractionsCount(InteractionsCountRequestProto request) {
        ...
        log.info("Рассчет суммы взаимодействий для {} мероприятий", sumOfScoresByEvent.size());
        return recommendationsMapper.mapToProto(sumOfScoresByEvent);
    }

    private Map<Long, Double> predictScores(List<EventSimilarity> candidates, List<Long> recentUserEvents,
                                            Map<Long, Double> userScores, int maxResults) {
        ...
        for (EventSimilarity candidate : candidates) {
            long candidateId = recentUserEvents.contains(candidate.getEventA())
                    ? candidate.getEventB() : candidate.getEventA();
